import CorrectionDemande from '../models/CorrectionDemande.js';
...
export const getAll = async (req, res) => {
  try {
    ...
    const items = await CorrectionDemande.find(query).populate('employe');
...
export const getById = async (req, res) => {
  try {
    const item = await CorrectionDemande.findById(req.params.id).populate('employe');
...
export const create = async (req, res) => {
  try {
    const item = new CorrectionDemande(req.body);
    await item.save();
...
export const update = async (req, res) => {
  try {
    const item = await CorrectionDemande.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item) return res.status(404).json({ success: false, message: 'Introuvable' });
...
export const remove = async (req, res) => {
  try {
    const item = await CorrectionDemande.findByIdAndDelete(req.params.id);
    if (!item) return res.status(404).json({ success: false, message: 'Introuvable' });
