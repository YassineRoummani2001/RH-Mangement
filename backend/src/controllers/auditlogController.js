...
export const getAll = async (req, res) => {
  try {
    ...
    const items = await AuditLog.find(query).populate('employe');
...
export const getById = async (req, res) => {
  try {
    const item = await AuditLog.findById(req.params.id).populate('employe');
...
export const update = async (req, res) => {
  try {
    const item = await AuditLog.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item) return res.status(404).json({ success: false, message: 'Introuvable' });
