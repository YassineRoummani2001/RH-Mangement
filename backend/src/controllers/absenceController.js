...
export const getAll = async (req, res) => {
  try {
    ...
    const items = await Absence.find(query).populate('employe');
...
export const getById = async (req, res) => {
  try {
    const item = await Absence.findById(req.params.id).populate('employe');
...
export const update = async (req, res) => {
  try {
    const item = await Absence.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item) return res.status(404).json({ success: false, message: 'Introuvable' });
...
export const remove = async (req, res) => {
  try {
    const item = await Absence.findByIdAndDelete(req.params.id);
    if (!item) return res.status(404).json({ success: false, message: 'Introuvable' });
